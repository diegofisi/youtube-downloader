...
export const LibraryListContainer = ({ onDeleteFile, onClearAll }: LibraryListContainerProps) => {
  ...
  return (
    <Stack gap="md">
      <LibraryToolbar search={search} countText={countText} onSearchChange={onSearchChange} onClearAll={onClearAll} />
      ...
      {entries !== undefined && filtered.length === 0 && (
        <PageEmpty
          message={
            entries.length === 0
              ? t(
                  'Sin descargas todavía. Lo que descargues aparecerá aquí.',
                  'No downloads yet. Your downloads will show up here.',
                )
