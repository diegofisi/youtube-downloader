...
export const QueueStats = ({ active, waiting, done, errors }: QueueCounts) => (
  <Stack direction="row" gap="none" wrap className="gap-3">
    <StatBox icon={DownloadIcon} toneClass="bg-primary-soft text-primary" value={active} label={t('Activas', 'Active')} />
    <StatBox icon={ListVideoIcon} toneClass="bg-info-soft text-info" value={waiting} label={t('En espera', 'Waiting')} />
    <StatBox icon={CheckIcon} toneClass="bg-success-soft text-success" value={done} label={t('Completadas', 'Completed')} />
    <StatBox
