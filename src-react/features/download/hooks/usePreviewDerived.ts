...
export function usePreviewDerived() {
  ...
  const summary = useMemo(() => {
    const selectableUrls = flat
      .filter((v) => STATUS_META[statusOf(v, downloaded)].downloadable)
      .map((v) => v.url);
    const chosen = flat.filter(
    ...
    const customCount = chosen.filter(
      (v) => overrides[v.url] && Object.keys(overrides[v.url]).length > 0,
    ).length;
