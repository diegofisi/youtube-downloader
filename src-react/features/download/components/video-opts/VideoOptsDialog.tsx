...
export const VideoOptsDialog = ({
  ...
  const setKey = <K extends keyof DownloadOpts>(k: K, v: DownloadOpts[K]) =>
    setDraft((d) => ({ ...d, [k]: v }));

  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[384px] gap-0 p-0">
        <Stack direction="row" gap="sm" align="center" className="gap-[10px] border-b border-border px-[17px] py-[15px]">
          <SettingsIcon className="size-[17px] flex-none text-primary" />
          <DialogTitle className="text-[14.5px]">
            {t('Opciones de este video', 'Options for this video')}
          </DialogTitle>
        </Stack>
        ...
        <Stack gap="md" className="max-h-[60vh] overflow-y-auto px-[17px] py-[15px]">
          ...
          {eff.mode !== 'audio' && (
            <>
              <Stack gap="xs">
                ...
                <ChipGroup options={qualityChips()} value={eff.quality} onChange={(quality) => setKey('quality', quality)} />
              </Stack>
              <Stack gap="xs">
                <Small className="text-[11.5px] font-semibold text-muted-foreground">
                  {t('Formato del archivo', 'File format')}
                </Small>
                <ChipGroup options={containerChips()} value={eff.container} onChange={(container) => setKey('container', container)} />
              </Stack>
            </>
          )}
          ...
          {eff.mode === 'audio' && (
            <>
              <Stack gap="xs">
                ...
                <ChipGroup options={audioFmtChips()} value={eff.audioFmt} onChange={(audioFmt) => setKey('audioFmt', audioFmt)} />
              </Stack>
              <Stack gap="xs">
                <Small className="text-[11.5px] font-semibold text-muted-foreground">
                  {t('Calidad de audio', 'Audio quality')}
                </Small>
                <ChipGroup options={bitrateChips()} value={eff.bitrate} onChange={(bitrate) => setKey('bitrate', bitrate)} />
              </Stack>
            </>
          )}
          ...
          <Box className="border-t border-border pt-3">
            ...
            >
              <ChevronRightIcon
                className={cn('size-[13px] flex-none transition-transform', advOpen && 'rotate-90')}
              />
              {t('Avanzado', 'Advanced')}
            </button>
