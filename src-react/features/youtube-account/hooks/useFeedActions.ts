...
/** Download/customize flows for the My YouTube grid (this slice's port of shared/ui/dl-actions).
 * Cross-feature access sanctioned by the R2 pinned contracts (queue/download/session). */
export function useFeedActions(onSelectionDone: () => void) {
  ...
  const toQueueItem = (v: FeedVideo): EnqueueItem => ({
    url: v.url,
    videoId: v.id,
    title: v.title,
    channel: v.channel,
  ...
  const downloadSelected = (items: FeedVideo[]) => {
  ...
  const customizeSelected = (items: FeedVideo[]) => {
    if (!items.length) return;
